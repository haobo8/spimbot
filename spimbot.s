...
main:
        ...
        or	$t4,	$t4,	FEEDBACK_INT_MASK	  # enable feedback iterrupt
...
solve_puzzle:
	...
	bge $s1, 6, solve_puzzle_done
		...
		beqz $t0, wait_for_feedback

		sw $0, feedback_received

		la $t0, wordle_feedback_t
		mul $t1, $s1, 16
		add $a0, $t0, $t1
		jal is_solved
		bnez $v0, solve_puzzle_done

		la $t0, wordle_feedback_t
		mul $t1, $s1, 16
		add $t0, $t0, $t1
		sw $s0, 12($t0) #set prev

		la $t1, wordle_feedback_t
		mul $t2, $s1, 16
		add $t1, $t1, $t2
		move $s0, $t1 #set curr
